...
> New in 1.4.0: a dedicated **Motivation** section (💪) in the sidebar. It shows your personal saying for the day and gives you an **"Extra boost"** button for a fresh motivational line whenever you need a push — as many as you like, counted for the day.
>
> Recent: 1.3.0 made Focus tasks editable, copyable, reorderable (drag), with a collapsible "Done" section. 1.2.0–1.2.1 introduced the **Focus area** (checklists, notepad, daily personalized motivation) and **automatic versioned backups** on every save. Switch between Passwords and Focus in the sidebar. Focus holds checklists you tick off (with a progress bar), a free-form notepad, and a **daily personalized motivation**: each day a welcome screen greets you by name with one of 200 hand-written sentences. Everything in Focus is stored encrypted in the same vault. This release also adds **automatic versioned backups** on every save (the last 20 states are kept in a `backups` folder), so any change can be rolled back. Earlier releases: 1.1.3 fixed clipped dashboard cards; 1.1.2 fixed the encrypted backup export and hardened the core; 1.1.1 fixed data loss with two open windows; 1.1.0 added the bilingual German/English interface.
