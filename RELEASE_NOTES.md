...
> New in 1.3.0: the Focus checklists got a big upgrade. Task text is now **editable in place** (fix a typo without deleting the task), each task has a **copy** button, tasks can be **reordered by dragging** (within a list or across lists), and **ticked-off tasks move into a collapsible "Erledigt" section** so long lists stay tidy.
>
> Introduced in 1.2.0–1.2.1: a **Focus area** next to your passwords (checklists, notepad, daily personalized motivation), plus **automatic versioned backups** on every save. Switch between Passwords and Focus in the sidebar. Focus holds checklists you tick off (with a progress bar), a free-form notepad, and a **daily personalized motivation**: each day a welcome screen greets you by name with one of 200 hand-written sentences. Everything in Focus is stored encrypted in the same vault. This release also adds **automatic versioned backups** on every save (the last 20 states are kept in a `backups` folder), so any change can be rolled back. Earlier releases: 1.1.3 fixed clipped dashboard cards; 1.1.2 fixed the encrypted backup export and hardened the core; 1.1.1 fixed data loss with two open windows; 1.1.0 added the bilingual German/English interface.
